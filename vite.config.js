...
export default defineConfig(({ mode }) => {
  // Load env file based on `mode` (e.g., production, development)
  ...
  return {
    plugins: [
      ...
      dsv({
        processRow: (row) => ({
          ...
          cutoffTime: new Date(row.cutoffTime),
        }),
      }),
    ],
    worker: {
    ...
    // Inject all env variables to client side via define
    define: Object.entries(env).reduce((acc, [key, val]) => {
      // JSON.stringify the value properly
