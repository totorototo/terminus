...
import UphillGradientBreakdown from "./UphillGradientBreakdown.jsx";
...
vi.mock("./UphillGradientBreakdown.style.js", () => ({
...
describe("UphillGradientBreakdown", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("renders nothing when route data is empty", () => {
    setupStore();
    const { container } = render(<UphillGradientBreakdown />);
    expect(container).toBeEmptyDOMElement();
  });

  it("renders a row with distance for each uphill gradient band", () => {
    setupStore({
      cumulativeDistances: [0, 100, 200],
      slopes: [0, 3, 12],
    });

    render(<UphillGradientBreakdown />);
