  ...
  return (
    <div style={{ marginBottom: "20px" }}>
      ...
      <p style={{ fontSize: "0.9em", opacity: "0.8", marginBottom: "15px" }}>
        Test Web Worker performance under various high-load scenarios to verify UI remains responsive
      </p>
      ...
      {stressResults && (
        <div style={{
          ...
          borderRadius: "8px",
          border: stressResults.testSuite ? "2px solid #4a6741" : "1px solid #4a4a6a"
        }}>
          <h5 style={{ margin: "0 0 15px 0", color: stressResults.testSuite ? "#90EE90" : "#87CEEB" }}>
            {stressResults.testSuite ? "🎯 Full Stress Test Results" : `📊 ${stressResults.testType} Results`}
          </h5>
          {stressResults.testSuite ? (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "15px", marginBottom: "20px" }}>
                <div style={{ textAlign: "center", padding: "10px", background: "#2a4a2a", borderRadius: "5px" }}>
                  <div style={{ fontSize: "1.5em", fontWeight: "bold", color: "#90EE90" }}>
              ...
              <div style={{ fontSize: "0.9em" }}>
                ...
                {Object.entries(stressResults.results).map(([testName, result]) => (
                  <div key={testName} style={{ marginBottom: "10px", padding: "8px", background: "#1a2a1a", borderRadius: "3px" }}>
                    <strong>{testName}:</strong> {result.successfulRequests || result.successfulTests || result.successfulSteps || 'completed'} successful
                    {result.avgProcessingTime && ` | Avg: ${result.avgProcessingTime.toFixed(2)}ms`}
                    {result.estimatedFPS && ` | FPS: ${result.estimatedFPS.toFixed(1)}`}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "10px", marginBottom: "15px" }}>
                {stressResults.totalRequests && (
                  <>
                    <div style={{ textAlign: "center", padding: "8px", background: "#2a3a2a", borderRadius: "3px" }}>
                      <div style={{ fontWeight: "bold" }}>{stressResults.successfulRequests}</div>
                      <div style={{ fontSize: "0.8em" }}>Successful</div>
                    </div>
                    <div style={{ textAlign: "center", padding: "8px", background: "#3a2a2a", borderRadius: "3px" }}>
                      <div style={{ fontWeight: "bold" }}>{stressResults.failedRequests}</div>
                      <div style={{ fontSize: "0.8em" }}>Failed</div>
                    </div>
                  </>
                )}
                {stressResults.avgProcessingTime && (
                  <div style={{ textAlign: "center", padding: "8px", background: "#2a2a3a", borderRadius: "3px" }}>
                    <div style={{ fontWeight: "bold" }}>{stressResults.avgProcessingTime.toFixed(2)}ms</div>
                    <div style={{ fontSize: "0.8em" }}>Avg Time</div>
                  </div>
                )}
                {stressResults.estimatedFPS && (
                  <div style={{ textAlign: "center", padding: "8px", background: "#2a2a3a", borderRadius: "3px" }}>
                    <div style={{ fontWeight: "bold", color: stressResults.isResponsive ? "#90EE90" : "#ff6b6b" }}>
              ...
              {stressResults.frameDropPercentage !== undefined && (
                <p style={{ margin: "0", fontSize: "0.9em" }}>
                  ...
                  UI Responsive: <span style={{ color: stressResults.isResponsive ? "#90EE90" : "#ff6b6b" }}>
