...
/// Per-point pace factors for one trace point: the slope-only Minetti term
/// (`terrain`) and the fully composed multiplier (`combined`) that also folds
/// in fatigue, circadian and weather. `terrain` alone drives effort-weighted
/// distance accounting (d_eff, totalWeightedDist); `combined` drives segment time.
pub const PaceFactors = struct {
    terrain: f64,
    combined: f64,
};

/// Compose the full per-point pace multiplier from its constituent factors —
/// the single point where the Minetti slope term joins fatigue, circadian and
/// weather to produce the multiplier actually applied to base pace.
///
/// `slope_frac`: rise/run fraction, fed to the Minetti metabolic-cost model.
/// `d_eff_km`: cumulative effort-weighted distance so far, in km (fatigue input).
/// `k_fatigue`: fatigue coefficient (see K_FATIGUE).
/// `clock_start`: race start epoch, or null for a neutral (1.0) circadian factor.
/// `now_s`: elapsed moving-time clock at this point, in seconds (added to clock_start).
/// `weather_factor`: precomputed weatherFactor() for the segment — weather is
/// constant per segment, so callers compute it once outside the per-point loop.
pub fn computeFactors(
    slope_frac: f64,
    d_eff_km: f64,
    k_fatigue: f64,
    clock_start: ?i64,
    ...
    const terrain = minetti.paceFactor(slope_frac);
    const fatigue = fatigueFactor(d_eff_km, k_fatigue);
    const circadian = if (clock_start) |t0|
        circadianFactor(t0 + @as(i64, @intFromFloat(now_s)))
    else
        1.0;
    return .{
        .terrain = terrain,
...
/// Computed once per segment (weather is constant across it) and folded into the
/// per-point multiplier by computeFactors — see its `weather_factor` parameter.
...
test "computeFactors: flat, fresh, no clock, neutral weather is fully neutral" {
    const f = computeFactors(0.0, 0.0, K_FATIGUE, null, 0.0, 1.0);
    try std.testing.expectApproxEqAbs(1.0, f.terrain, 1e-9);
    try std.testing.expectApproxEqAbs(1.0, f.combined, 1e-9);
}

test "computeFactors: terrain matches minetti.paceFactor for the given slope" {
    const f = computeFactors(0.10, 0.0, K_FATIGUE, null, 0.0, 1.0);
    try std.testing.expectApproxEqAbs(minetti.paceFactor(0.10), f.terrain, 1e-9);
}

test "computeFactors: combined is the product of all constituent factors" {
    const slope_frac = 0.08;
    const d_eff_km = 50.0;
    const clock_start: i64 = 1_700_000_000;
    const now_s: f64 = 3600.0;
    const weather_factor = 1.05;

    const f = computeFactors(slope_frac, d_eff_km, K_FATIGUE, clock_start, now_s, weather_factor);

    const expected = minetti.paceFactor(slope_frac) *
        fatigueFactor(d_eff_km, K_FATIGUE) *
        circadianFactor(clock_start + @as(i64, @intFromFloat(now_s))) *
        weather_factor;
    try std.testing.expectApproxEqAbs(expected, f.combined, 1e-12);
}

test "computeFactors: null clock_start leaves circadian neutral" {
    const with_clock = computeFactors(0.0, 0.0, K_FATIGUE, 12 * 3600, 0.0, 1.0);
    const without_clock = computeFactors(0.0, 0.0, K_FATIGUE, null, 0.0, 1.0);
    // Noon UTC is itself circadian-neutral, so both should agree here...
    try std.testing.expectApproxEqAbs(with_clock.combined, without_clock.combined, 1e-9);
    // ...but at the circadian peak the two must diverge.
    const peak_clock = computeFactors(0.0, 0.0, K_FATIGUE, 3 * 3600 + 30 * 60, 0.0, 1.0);
    try std.testing.expect(peak_clock.combined > without_clock.combined);
}
